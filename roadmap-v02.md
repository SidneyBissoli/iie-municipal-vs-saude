...
**Versão:** v02 · gerada a partir de `proposta-de-pesquisa-v02.md` (em redação) · sucede `roadmap-v01.md`
...
- [x] **(P0)** Estabelecer convenção de **ADRs** em `decisions/`. Convenção
  ...
  - **Pré-mapeados:** oito ADRs identificados pelos itens 🜲 — ADR-001
    ...
    **Atualização v02 (2026-05-07).** ADR-008 (viabilidade de
    negative controls) **descontinuada** na v02 da proposta. O
    bullet de §3.3 do roadmap-v02 agora operacionaliza Não-ICSAP
    no Desenho A como negative control outcome obrigatório
    (Lipsitch et al., 2010), conforme briefing v02 §2.4. A
    pergunta de viabilidade está resolvida positivamente; não há
    decisão pendente a registrar como ADR. Sete ADRs
    pré-mapeadas remanescentes: ADR-001, ADR-002, ADR-003,
    ADR-004, ADR-005, ADR-007, ADR-009. ADR-006 já materializada
    (errata interpretativa do ADR-005, sessão 010).
...
- [ ] **(P0)** Comparar magnitude do β entre Desenho A e Desenho B — testar
  hipótese H4.
- [ ] **(P0)** **ETWFE/Mundlak (Wooldridge, 2025)** como estimação adicional
  do Desenho B, blindada contra heterogeneidade temporal entre coortes:
  `fixest::fepois` com inclusão manual de médias intra-UF e intra-coorte do
  IIE como controles. Reportada quantitativamente na mesma tabela do TWFE
  Poisson principal. Ancorada em ADR-005 e na nota de pesquisa
...
- [ ] **(P0)** **Não-ICSAP no Desenho A — negative control outcome
  obrigatório:** estimar efeito do IIEM sobre internações **não** sensíveis à
  atenção primária na faixa 20–29 anos, via mesmo modelo Poisson com offset do
  Desenho A. Lógica de negative control (Lipsitch et al., 2010): se o IIEM
  realmente afeta desfechos sensíveis à APS, o efeito sobre Não-ICSAP deve ser
  nulo ou substantivamente menor. Restrita ao Desenho A; reportada em apêndice
  (1–2 pp.). Substitui a "IT total" inicialmente cogitada, que continha o sinal
  ICSAP. Resolve positivamente a pergunta da ADR-008 pré-mapeada
  (descontinuada na v02 — ver §0.5).
- [ ] **(P1)** Análise de **specification curve** (Simonsohn et al., 2020) —
  ...
  β.
- [ ] **(P0)** **Sensibilidade S1 — painel UF restrito sem 2020-2021:**
  re-estimar Desenho B excluindo as coortes/anos de pico pandêmico. Avalia
  robustez ao choque agregado COVID-19 sobre os sistemas SIM/SIH e sobre a
  função reduzida estimada. Reportada quantitativamente na mesma tabela do
  TWFE Poisson principal (§3.1).
- [ ] **(P0)** **Sensibilidade S2 — estratificação temporal por subperíodos:**
  estimar coeficiente de interesse separadamente em 2018-2019 (pré-pandemia) e
  2022-2024 (pós-pandemia), conforme disponibilidade do IIE estadual nas
  coortes correspondentes. Avalia estabilidade temporal do efeito além do
  simples controle pelo choque. Reportada quantitativamente na mesma tabela.
- [ ] **(P0)** **Tendências lineares por UF (state-specific linear trends):**
  especificação alternativa do Desenho B com `α_u + γ_c + α_u·t`, absorvendo
  tendências UF-específicas além dos efeitos fixos two-way. Sensibilidade ao
  pressuposto de parallel trends; reportada quantitativamente.
- [ ] **(P0)** **Exclusão da coorte 2019 do Desenho B (ADR-005):** re-estimar
  painel UF no subconjunto coorte ∈ {2013, 2015, 2017} (n = 81 observações),
  implementação em `analysis/desenho_b_sens_no2019.R` (conforme ADR-005
  §Decisão). Sensibilidade central, não marginal: testa se conclusões dependem
  da camada adicional de imputação 2017→2019 carregada apenas pela coorte
  2019 (Fernandes, Felicio & Saad, 2025; análise crítica em
  `bibliography/research-notes/saeb-2019-idade.md`).
